...
def save_preprocessing_objects(scaler, label_encoders, imputer, removed_cols, output_dir):
    ...
    joblib.dump(removed_cols, os.path.join(output_dir, 'removed_cols.pkl'))
...
def split_data(df, target, test_size=0.2, random_state=42):
    logging.info(f"Splitting data into train and test sets with test_size={test_size} and random_state={random_state}...")
...
def balancing(X_train, y_train, random_state=42):
    logging.info(f"Balancing the training data using SMOTE with random_state={random_state}...")
...
def main(data_dir, output_dir, target_col, random_state, columns_to_remove):
    ...
    X_train_balanced, y_train_balanced = balancing(X_train, y_train, random_state=random_state)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Prepare and clean data for machine learning.")
    parser.add_argument('--data_dir', type=str, required=True, help="Directory where the data is stored and processed.")
